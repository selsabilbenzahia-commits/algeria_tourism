...
<head>
    ...
    <title><?php echo $texts[$lang]['att_title']; ?></title>
    ...
    <style>
        ...
        .filter-btn { background: #fff; border: 2px solid #eee; padding: 10px 22px; border-radius: 25px; font-family: 'Cairo', sans-serif; font-weight: bold; cursor: pointer; transition: 0.3s; color: #555; }
        .filter-btn.active, .filter-btn:hover { background: #c5a059; color: white; border-color: #c5a059; box-shadow: 0 4px 15px rgba(197, 160, 89, 0.2); }
    </style>
</head>
<body>
...
<section class="all-wilayas-page">
    <div class="container">
        ...
        <header class="page-header" style="margin-bottom: 40px; text-align: center;">
            <h1 class="page-title"><?php echo $texts[$lang]['att_title']; ?></h1>
            <div class="title-line" style="width: 80px; height: 3px; background: #c5a059; margin: 10px auto;"></div>
        </header>

        <div class="filter-container">
            <button class="filter-btn active" data-filter="all"><?php echo $texts[$lang]['filter_all']; ?></button>
            <?php 
            $cat_query = "SELECT * FROM categories";
            $cat_res = mysqli_query($conn, $cat_query);
            while($cat_row = mysqli_fetch_assoc($cat_res)) {
                echo '<button class="filter-btn" data-filter="cat-' . $cat_row['id'] . '">' . $cat_row['name_'.$lang] . '</button>';
            }
            ?>
        </div>
        ...
        <div class="wilayas-grid">
            ...
            ?>
                <div class="wilaya-card <?php echo $card_size . ' ' . $cat_class; ?>">
