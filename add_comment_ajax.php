...
if (isset($_POST['comment_text']) && isset($_SESSION['user_id'])) {
    ...
    // 1. جلب اسم المستخدم من قاعدة البيانات بدل السشن
    $user_query = mysqli_query($conn, "SELECT name FROM users WHERE id = '$user_id'");
    $user_data = mysqli_fetch_assoc($user_query);
    $user_name = $user_data['name'] ?? 'Guest'; // إذا مالقاش الاسم يدير Guest

    // 2. كود الإدخال (تأكد من الـ IDs)
    if (isset($_POST['id'])) {
        $id = $_POST['id'];
    ...
    if (mysqli_query($conn, $sql)) {
        // نرجعو الـ HTML الصافي بلا Warnings
